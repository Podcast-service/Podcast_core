...
Открытие карточки не увеличивает `viewsCount`: просмотр засчитывается только через сохранение положительного прогресса прослушивания.
...
Возвращает сохранённый транскрипт выпуска. Для subtitle в `content` находится массив последовательных блоков `text`/`voice`, подготовленный при обработке Kafka-события. Коды: `200`, `404`.
...
Сохраняет прогресс прослушивания текущего пользователя. Первый положительный `progressSeconds` для пары пользователь + опубликованный подкаст атомарно увеличивает `viewsCount` на 1. Повторные вызовы того же пользователя, а также `progressSeconds = 0`, не увеличивают счётчик.
