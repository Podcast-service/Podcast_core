...
| `404` | `AUTHOR_PROFILE_NOT_FOUND` | у пользователя нет локального профиля автора | открыть сценарий становления автором |
| `404` | `PLAYLIST_NOT_FOUND` | плейлист отсутствует или недоступен текущему пользователю | показать состояние “не найдено” |
| `409` | `CONFLICT` | конфликт уникальности или состояния | обновить данные и повторить действие |
| `409` | `ALREADY_SAVED` | плейлист уже сохранён в библиотеку пользователя | обновить состояние кнопки сохранения |
| `422` | `BUSINESS_RULE_VIOLATION` | бизнес-правило запрещает операцию | показать причину пользователю |
| `403` | `CANNOT_SAVE_OWN_PLAYLIST` | пользователь пытается сохранить собственный плейлист | скрыть действие для собственных плейлистов |
