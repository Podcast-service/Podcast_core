...
Для `media.worker` поддерживаются только события `start_processing`, `processed`, `processing_failed` и `error`. События вне этого набора, включая промежуточные события стороннего worker-а, не обрабатываются и не меняют статус подкаста.

`duration_seconds` и `audio_file_size` принимаются как JSON number или строка с целым числом. Дробные значения не входят в контракт и отклоняются как невалидное сообщение.

Consumer не выводит `object_type`, `event` или идентификатор подкаста из других полей и не использует Kafka key как замену обязательным полям payload.
...
Если `podcast_id` отсутствует, сообщение считается невалидным и не изменяет данные.
