...
`listen_history.view_counted` фиксирует, был ли уже засчитан просмотр в `podcasts.views_count` для пары пользователь + подкаст. Колонка внутренняя и не входит в публичные DTO.
...
`views_count` увеличивается не при чтении `GET /podcasts/{podcastId}`, а при первом положительном прогрессе через `POST /podcasts/{podcastId}/progress`. Инкремент выполняется в одной транзакции с обновлением `listen_history`.
