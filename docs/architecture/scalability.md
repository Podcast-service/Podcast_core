...
Масштабирование consumer зависит от количества partitions в topics `podcast.user.register`, `media.upload`, `media.worker`, `media.subtitle`, `tts.start` и `tts.failed`. Один partition обрабатывается только одним экземпляром внутри consumer group.
